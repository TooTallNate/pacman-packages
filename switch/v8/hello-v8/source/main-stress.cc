...
static void RunV8() {
  ...
  }
  {
    ...
    // --always-sparkplug compiles every function to native baseline (Sparkplug)
    // code, so these exercise real JIT codegen across varied workloads.
    run_case("int-loop",
             "function f(){let s=0;for(let i=0;i<100000;i++)s=(s+i)|0;return s;}"
             "f();f();", "704982704");
    run_case("float-math",
             "function g(x){return Math.sqrt(x*x+1.0)*2.5;}"
             "g(3.0);g(3.0).toFixed(4);",
             "7.9057");
    run_case("strings",
             "function h(n){let s='';for(let i=0;i<n;i++)s+=('ab'+i);return "
             "s.length;}h(3);h(100);", "390");
    run_case("array-sort",
    ...
    run_case("try-catch",
             ...
             "catch(e){return -1;}}t(5);"
             "''+t(5)+','+t(-1);", "10,-1");
    run_case("json",
             "var o={a:1,b:[2,3],c:'x'};JSON.stringify(JSON.parse("
             "JSON.stringify(o)));", "{\"a\":1,\"b\":[2,3],\"c\":\"x\"}");

    {
      char buf[96];
      snprintf(buf, sizeof(buf), "\nstress battery: %d failures\n", g_fail);
      locked_print(buf);
    }
    g_ok = (g_fail == 0) ? 1 : 0;
  }
...
int main(int argc, char* argv[]) {
  ...
  locked_print(g_ok ? "\nSTRESS: all JIT tests passed!\nPress + to exit.\n"
                    : "\nSTRESS: some JIT tests FAILED\nPress + to exit.\n");
